...
The included analysis backend is CSA build `v4.27.0-structure-led-local-impulse`. Benchmark package v2.25 inventories the immediately previous completed framework period first and the period before it second, checking S/R and S/D within each period before older structure may influence the controlling impulse. Among completed impulses that explain nearby-period structure, recency now outranks the wider match count so a stale broad frame cannot displace the nearer completed leg. When screenshot-only analysis is required, the selector independently scans every horizontal line—including closely stacked lines—and may replace an over-broad reported impulse only when a nearer exact structural origin validates at least two separate levels. Separately printed converted S/R lines remain separate, and ordinary price-axis ticks cannot become zone boundaries. The selector supports up to three independently qualified alternative entries, retains broker-index aliases and time-axis reconciliation, and keeps the uploaded screenshot authoritative in benchmark dry-run mode. The batch-testing additions do not alter the customer analysis route.
