...
The included analysis backend is CSA build `v4.25.0-targeted-sd-fib-recovery`. Benchmark package v2.23 keeps the stable v2.22 selector, adds a focused recovery pass only for independently evidenced supply/demand zones missed by the main read, and verifies every recovered candidate against server-calculated 38.2%, 50%, and 61.8% prices. It does not merge extra support/resistance candidates or replace the primary completed impulse. It supports up to three independently qualified alternative entries, retains broker-index aliases and time-axis reconciliation, and keeps the uploaded screenshot authoritative in benchmark dry-run mode. The batch-testing additions do not alter the customer analysis route.
