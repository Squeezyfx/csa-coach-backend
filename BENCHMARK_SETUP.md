...
The included analysis backend is CSA build `v4.28.0-local-frame-stacked-line-audit`. Benchmark package v2.26 inventories the immediately previous completed framework period first and the period before it second, checking S/R and S/D within each period before older structure may influence the controlling impulse. Among completed impulses that explain nearby-period structure, recency now outranks the wider match count so a stale broad frame cannot displace the nearer completed leg. A nearer chart-native impulse may also replace a broad reported impulse when it produces a materially closer, independently marked Fibonacci-qualified level. For closely stacked converted S/R lines, the engine performs a narrow second visual read and adds a level only when a separate printed chart label is actually confirmed; Fibonacci never creates the level. Separately printed converted S/R lines remain separate, and ordinary price-axis ticks cannot become zone boundaries. The selector supports up to three independently qualified alternative entries, retains broker-index aliases and time-axis reconciliation, and keeps the uploaded screenshot authoritative in benchmark dry-run mode. The batch-testing additions do not alter the customer analysis route.
