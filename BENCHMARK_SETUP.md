...
The included analysis backend is CSA build `v4.17.0-structure-guided-impulse-selection`. Benchmark package v2.15 keeps broker-index aliases and time-axis reconciliation, reconciles exact chart structure before selecting the completed impulse used for hidden Fibonacci validation, and requires independent evidence before exposing Entry 2. The batch-testing additions do not alter the customer analysis route.
