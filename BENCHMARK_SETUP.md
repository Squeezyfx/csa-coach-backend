...
The included analysis backend is CSA build `v4.30.0-prior-level-conversion-compatibility`. Benchmark package v2.28 inventories the immediately previous completed framework period first and the period before it second, checking S/R and S/D within each period before older structure may influence the controlling impulse. When that preserved inventory proves a nearer completed local impulse, that one shared Fibonacci frame overrides a broad frame that would skip the nearer valid structure. Exact printed S/R stays an exact price—only genuine supply/demand uses zone boundaries—so a farther line cannot inherit Fibonacci confluence from an artificial width. A printed prior support/resistance that price clearly continues through is retained as a converted level when a separately confirmed converted sibling proves the same break-and-hold regime; this preserves both valid nearby USA30 alternatives. For closely stacked converted S/R lines, the engine performs a narrow second visual read and adds a level only when a separate printed chart label is actually confirmed; Fibonacci never creates the level. The selector supports up to three independently qualified alternative entries, retains broker-index aliases and time-axis reconciliation, and keeps the uploaded screenshot authoritative in benchmark dry-run mode. The batch-testing additions do not alter the customer analysis route.
