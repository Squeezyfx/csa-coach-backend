...
The included analysis backend is CSA build `v4.18.0-chart-native-fallback-entry-dedupe`. Benchmark package v2.16 keeps broker-index aliases and time-axis reconciliation, gives exact chart-visible framework prices precedence over nearby inferred fragments, requires independent evidence before exposing Entry 2, and adds a tightly validated chart-native fallback when cutoff-filtered market candles are unavailable. The batch-testing additions do not alter the customer analysis route.
