...
The included analysis backend is CSA build `v4.26.0-chronological-period-inventory`. Benchmark package v2.24 inventories the immediately previous completed framework period first and the period before it second, checking S/R and S/D within each period before older structure may influence the controlling impulse. Impulse ranking now prefers the frame that explains more structure from those two nearest periods before using recency. It also preserves separately printed converted S/R lines when a visual pass incorrectly combines them into a broad supply/demand zone, and prevents ordinary price-axis ticks from becoming zone boundaries. It supports up to three independently qualified alternative entries, retains broker-index aliases and time-axis reconciliation, and keeps the uploaded screenshot authoritative in benchmark dry-run mode. The batch-testing additions do not alter the customer analysis route.
