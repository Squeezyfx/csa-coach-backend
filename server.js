...
function buildSimpleMistakeHub({
  failedAreas = [],
  hasConfirmedTrigger = false,
  ...
  const add = (title, severity) => {
    ...
    const alreadyExists = items.some(
      (item) => String(item.title).toLowerCase() === cleanTitle.toLowerCase()
    );
  ...
  if (!marketOk) {
    add("Market data unavailable", "DATA ISSUE");
  }

  if (chartDetection?.isTradingChart && chartDetection?.hasUsablePriceData === false) {
    add("Chart data unclear", "REVIEW");
  }

  if (!hasConfirmedTrigger) {
    add("Failed to wait for confirmation", "DISCIPLINE");
  }

  if (rejectedContext && !hasConfirmedTrigger) {
    add("Entered too early", "HIGH RISK");
  }

  if (mixedBias) {
    add("Entered in unclear structure", "STRUCTURAL");
  }

  failedAreas.slice(0, 4).forEach((area) => {
    add(simpleFailedAreaTitle(area), "STRUCTURAL");
  });

  if (Number(setupScore) > 0 && Number(setupScore) < 6) {
    add("Low-quality setup", "WARNING");
  }

  if (Number(entryAccuracyScore) > 0 && Number(entryAccuracyScore) < 50) {
    add("Entry accuracy weak", "WARNING");
  }

  if (Number(riskManagementScore) > 0 && Number(riskManagementScore) < 55) {
    add("Risk-to-reward below plan", "MATH FLAW");
  }

  if (items.length === 0) {
    add("No major mistake detected", "REVIEW");
  }
...
function buildDashboardAliases(dashboardFeedback = {}) {
  ...
  const aiMistakeDetectionHub =
    ...
    dashboardFeedback.aiMistakeDetectionHub.length
      ...
      : [makeSimpleMistake("No major mistake detected", "REVIEW")];
