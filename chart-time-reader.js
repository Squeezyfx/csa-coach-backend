...
function barsBetween(start,end,minutes){let time=start,count=0;while(time<end&&count<10000){time=advance(time,1,minutes);count++;}return time===end?count:null;}
...
export function resolveVisibleTimestampFromAxisCount({timeAxisTimestamps=[],visibleCandlesAfterLastPrintedDate,timeframe}={}){
 ...
 for(let i=1;i<rows.length;i++){const bars=barsBetween(rows[i-1],rows[i],minutes);if(bars===null||bars<1)return null;}
 const timestamp=new Date(advance(rows.at(-1),count,minutes)).toISOString().slice(0,19).replace("T"," ");
