...
export function buildVisiblePeriodFibonacciFrame({ candles = [], direction = "range", timeframe = "" } = {}) {
  ...
  const bias = resolveFrameworkBias({
    periodInventory: [{high:swingHigh,low:swingLow,partialPeriod:true}],
    periodOpen, periodClose,
  });
  if (!bias) return null;
  const periodCandleDirection = bias.direction;
  const effectiveDirection = bias.direction;
