...
export function buildVisiblePeriodFibonacciFrame({ candles = [], direction = "range", timeframe = "" } = {}) {
  ...
  // frame is the full calendar year from January 1), we must not silently
  // build a frame from partial data. Return null so the caller treats this
  // as "Needs review" instead of guessing an incomplete year/month high-low.
